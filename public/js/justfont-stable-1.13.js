(function(window, document, undefined){
	...
	var justFont = {
			addClass: _jf["addClass"],
			removeClass: _jf["removeClass"],
	  ...
	  , STATE_LOADING = "jf-loading"
	  , STATE_ACTIVE = "jf-active"
	  , STATE_INACTIVE = "jf-inactive"
	  ;
	delete(_jf["addClass"]);
	delete(_jf["removeClass"]);
	delete(_jf["addScript"]);
	justFont.addClass( STATE_LOADING );
	...
	(function(window) {
	    ...
	    function ajax(method, url, data, headers) {
	        ...
	        var timeout = justFont.promise.ajaxTimeout
	          , timer;
	        headers = headers || {};
	        ...
	        if( window.XDomainRequest ){
	        	...
	        	xhr.ontimeout = function(){
	        		p.done(justFont.promise.ETIMEOUT, "");
	        		xhr.ontimeout = xhr.onerror = xhr.onload = xhr.onprogress = null;
	        	};
	        	xhr.onerror = function(){
	        		p.done(justFont.promise.EONERROR, xhr.responseText);
	        		xhr.ontimeout = xhr.onerror = xhr.onload = xhr.onprogress = null;
	        	};
	        	xhr.onload = function(){
	...
	fontContainer.prototype = {
		...
		addSelectors: function( selectors ) {
			this.fontSelectors.push( selectors );
		},
	  ...
	  , builder = function( _jf ){
			  ...
			  , events = {
	  		  ...
	  		  , parseMethod = {
					...
					"_event": function(key, callback){
						if( events[key] && key != 'Preload' ) {
			  ...
			  , triggerEvent = function(eventName){
				  	for(var i in events[eventName]){
						if( false === events[eventName][i].call() ) return ;
					}
					events[eventName] = [];
			  	}
			  ...
			  , getAjaxRecept = function(){
			  		var url = "http://dhs.justfont.com/post/recept/"
			  		  , data = getFontsData()
			  		  ;
					...
					justFont.addScript(url, 'text/javascript', true, 2000, function(){
					}, function(){
			  ...
			  , generateFontStyle = function(data){
			  		...
			  		for(var fontName in data) {
			  			...
			  			aliasList = font.getFontAlias() || [ fontName ];
			  ...
			  , loadFont = function(){
			  		...
			  		justFont.promise.join(tasks).then(function(errors, values){
			  			var success = true;
			  			for(var i in errors){
			  				if( errors[i] ){
			  					success = false;
			  ...
			  , Push = function(){
			  		// TODO: Add API for push function on justfont object
			  		console.log("Not implement yet.");
				}
			  ;
			triggerEvent("Loaded");
			addEvent("Inactived", function(){
				justFont.removeClass( STATE_LOADING );
				justFont.addClass( STATE_INACTIVE );
			});
			addEvent("Actived", function(){
				justFont.removeClass( STATE_LOADING );
				justFont.addClass( STATE_ACTIVE );
			});
			parser( _jf );
			traverseFontAlias();
